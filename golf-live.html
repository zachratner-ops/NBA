...
.topbar{background:#1a1a1a;color:#fff;padding:12px 20px;display:flex;align-items:center;justify-content:space-between;gap:8px;position:sticky;top:0;z-index:50;}
...
.topbar-nav a{color:#aaa;font-size:13px;text-decoration:none;white-space:nowrap;}
...
/* Desktop draft viewport fit */
@media(min-width:701px){
  .draft-screen{display:flex;flex-direction:column;height:calc(100dvh - 41px);padding:12px 20px 0;overflow:hidden;}
  .draft-screen .timer-bar{flex-shrink:0;}
  .draft-screen .makeup-notice{flex-shrink:0;}
  .draft-screen .draft-layout{flex:1;min-height:0;margin-bottom:8px;}
  .draft-screen .card.draft-col{display:flex;flex-direction:column;min-height:0;}
  .draft-screen #pick-order-list{flex:1;min-height:0;max-height:none;overflow-y:auto;}
  .draft-screen .pool-list{flex:1;min-height:0;max-height:none;overflow-y:auto;}
  .draft-screen .ap-list{flex:1;min-height:0;max-height:none;overflow-y:auto;}
  .draft-screen .roster-grid{flex-shrink:0;padding-bottom:12px;}
}
...
.odds-pill{font-size:10px;font-weight:700;padding:3px 0;border-radius:4px;display:flex;flex-direction:column;align-items:center;gap:0px;line-height:1.2;width:46px;flex-shrink:0;text-align:center;}
.odds-pill .odds-label{font-size:8px;font-weight:600;opacity:0.75;text-transform:uppercase;letter-spacing:.04em;}
.dk{background:#1a5c2a;color:#fff;}
.odds-top10{background:#1a3a5c;color:#fff;}
.odds-cut{background:#5c1a1a;color:#fff;}
.pool-list{max-height:350px;overflow-y:auto;}
#pick-order-list{max-height:350px;overflow-y:auto;}
.ap-list{max-height:350px;overflow-y:auto;}
...
.results-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:12px;background:transparent;}
.result-card{background:#fff;padding:16px;border-radius:10px;border:0.5px solid #e0e0e0;}
...
.rc-round-meta{font-size:10px;color:#bbb;margin-bottom:6px;}
.round-score-box{display:flex;align-items:center;justify-content:center;gap:2px;background:#f5f5f3;border:1px solid #e0e0e0;border-radius:3px;padding:1px 5px;font-size:10px;color:#888;white-space:nowrap;}
.rc-player{font-size:12px;padding:4px 0;min-height:24px;display:grid;grid-template-columns:1fr 52px 44px;align-items:center;color:#555;border-bottom:0.5px solid #f5f5f3;gap:4px;}
...
.rc-player.counting{background:#e8f5e9;border-radius:3px;padding:2px 4px;margin:-2px -4px;border-bottom:1px solid #fff !important;}
.rc-player.alt{border-top:1.5px solid #eee;margin-top:3px;padding-top:3px;}
.rc-score-val{font-weight:600;text-align:right;}
...
.sub-btn-wrap{display:flex;padding:10px 0 0 0;}
.sub-btn-wrap .btn{width:100%;text-align:center;}
...
@media(max-width:700px){
  ...
  .pool-list{max-height:calc(100dvh - 220px);overflow-y:auto;}
  #pick-order-list{max-height:252px;overflow-y:auto;}
...
.rc-trend{display:flex;flex-direction:column;gap:1px;margin-top:8px;padding-top:8px;border-top:0.5px solid #f0f0f0;}
.rc-trend-row{font-size:10px;line-height:1.5;white-space:nowrap;}
...
<div class="topbar">
  ...
  <div class="topbar-nav">
    ...
    <a href="golf.html" class="topbar-hide-mobile">History</a>
    <a class="fb-link topbar-hide-mobile" href="#" onclick="event.preventDefault();openFeedback()">💡 Suggest</a>
  </div>
</div>
...
let subSwapOut = null;
let subModalRound = 1;
let lbSortMode = 'score'; // 'score' | 'pick'
...
onValue(liveRef(), snap => {
  live = snap.val();
  // Hydrate scoreHistory from Firebase on first load
  if (live?.scoreHistory && scoreHistory.length === 0) {
    const sorted = Object.values(live.scoreHistory).sort((a,b) => new Date(a.ts) - new Date(b.ts));
    scoreHistory = sorted.map(s => ({ ts: new Date(s.ts).getTime(), scores: s.scores }));
  }
  ...
  document.getElementById('screen').innerHTML = `
    <div class="main-narrow">
      <div class="card">
        ...
        <div style="display:flex;gap:8px;">
          <button class="btn btn-outline" style="color:#1a1a1a;border-color:#ccc;" onclick="window.setupStepBack()">← Back</button>
        ...
        <div style="margin-bottom:12px;">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            ...
            <div style="display:flex;align-items:center;gap:4px;">
              <input type="number" id="custom-timer-input" min="1" placeholder="min" style="width:60px;border:1px solid #ddd;border-radius:6px;padding:4px 8px;font-size:12px;text-align:center;">
              <button class="btn btn-sm btn-outline" style="color:#555;border-color:#ddd;" onclick="window.setCustomTimer()">Set</button>
            </div>
          </div>
          ${![120,300,600,7200].includes(lobbyTimer||7200) ? `<div style="font-size:11px;color:#2e7d32;margin-top:5px;">Custom: ${Math.floor(lobbyTimer/60)}m ${lobbyTimer%60>0?lobbyTimer%60+'s':''}</div>` : ''}
        </div>
...
function renderDraft() {
  ...
  const pickOrderHtml = allPicks
    .map((p, i) => ({ p, i }))
    .filter(({ i }) => isMobile ? i >= globalIdx : i >= Math.max(0, globalIdx - 3))
    .map(({ p, i }) => {
      const done = i < globalIdx, current = i === globalIdx, isA = i >= seq.length;
      return `<div class="pick-item ${done?'done':current?'current':'upcoming'}" ${current?'id="current-pick"':''}>
  ...
  const poolHtml = (draft.field || [])
    ...
    .map(p => `<div class="player-card ${selectedPlayer?.name===p.name?'selected':''}" onclick="window.selectPlayer(this.dataset.p)" data-p='${JSON.stringify(p).replace(/'/g,"&apos;")}'>
      ...
      <div class="odds-row">${p.odds_dk?`<span class="odds-pill dk"><span class="odds-label">Win</span>${p.odds_dk}</span>`:''}${p.odds_top10?`<span class="odds-pill odds-top10"><span class="odds-label">Top 10</span>${p.odds_top10}</span>`:''}${p.odds_cut?`<span class="odds-pill odds-cut"><span class="odds-label">Cut</span>${p.odds_cut}</span>`:''}</div>
    </div>`).join('');
    ...
    return `<div class="roster-col">
      ...
      ${Array(4).fill(0).map((_,i) => {
        ...
        return `<div class="roster-slot ${g?'':'empty'} ${isMakeupSlot?'makeup':''}">
          ...
          ${g?.pickNumber ? `<span style="font-size:9px;color:#aaa;margin-left:auto;">#${g.pickNumber}</span>` : ''}
        </div>`;
      }).join('')}
  ...
  document.getElementById('screen').innerHTML = `
    <div class="main draft-screen">
      <div style="flex-shrink:0;display:flex;gap:8px;justify-content:flex-end;align-items:center;padding-bottom:8px;">
        <button class="btn btn-sm btn-outline" onclick="window.doUndo()" ${(draft.currentPickIndex > 0 || draft.currentPhase === 'alternate') ? '' : 'disabled'} style="color:#1a1a1a;border-color:#ccc;">↩ Undo</button>
        <button class="btn btn-sm btn-outline" onclick="window.doRedo()" style="color:#1a1a1a;border-color:#ccc;">↪ Redo</button>
        ${isComm ? `<button class="btn btn-sm btn-red" onclick="document.getElementById('reset-modal').style.display='flex'">Reset Draft</button>` : ''}
        ${isComm ? `<button class="btn btn-sm btn-outline" style="color:#1a1a1a;border-color:#ccc;" onclick="window.importOdds()">Odds</button>` : ''}
      </div>
      ...
      <div class="draft-layout">
        ...
        <div class="card draft-col active">
          ...
          <div class="ap-list">${(draft.autopickList || [])
            ...
            .join('') || '<div style="color:#aaa;font-size:12px;padding:4px;">All assigned or none set</div>'}</div>
        </div>
      </div>
  ...
  if (window.innerWidth <= 700) {
    const el = document.getElementById('pick-order-list');
...
function renderLive(isFinal) {
  ...
  if (lbSortMode === 'pick') {
    const pickSeqOrder = (draft?.pickSequence || []).map(p => p.owner);
    lbData.sort((a, b) => (pickSeqOrder.indexOf(a.owner)+1||999) - (pickSeqOrder.indexOf(b.owner)+1||999));
  }
  ...
  const commHtml = isComm ? `
    <div class="comm-panel">
      ...
      <div class="comm-actions">
        ...
        ${subs.length > 0 ? `<button class="btn btn-sm btn-outline" style="color:#fff;border-color:#444;" onclick="window.undoSub()">↩ Undo Last Sub</button>` : ''}
        <button class="btn btn-sm btn-outline" style="color:#fff;border-color:#444;" onclick="window.refreshScores()">↻ Refresh Scores</button>
        <button class="btn btn-sm btn-outline" style="color:#fff;border-color:#444;" onclick="window.importOdds()">↻ Refresh Odds</button>
        <button class="btn btn-sm btn-outline" style="color:${showOverridePanel?'#e65100':'#aaa'};border-color:${showOverridePanel?'#e65100':'#444'};" onclick="window.toggleScoreOverride()">${showOverridePanel?'✎ Close Override':'✎ Override Score'}</button>
        ${!isFinal
      ...
      ${showOverridePanel ? `
      <div style="background:#111;border-top:1px solid #333;padding:10px 20px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
        <span style="font-size:11px;color:#888;font-weight:700;flex-shrink:0;">OVERRIDE SCORE</span>
        <input id="override-player" list="override-players" placeholder="Player name…" style="border:1px solid #444;background:#1a1a1a;color:#fff;border-radius:6px;padding:4px 8px;font-size:12px;width:180px;">
        <datalist id="override-players">
          ${Object.values(draft?.picks||{}).flatMap(p=>[...p.golfers.map(g=>g.name), p.alternate?.name].filter(Boolean)).map(n=>`<option value="${n}">`).join('')}
        </datalist>
        <input id="override-score" type="number" placeholder="e.g. -5" style="border:1px solid #444;background:#1a1a1a;color:#fff;border-radius:6px;padding:4px 8px;font-size:12px;width:90px;">
        <label style="display:flex;align-items:center;gap:4px;color:#aaa;font-size:11px;cursor:pointer;"><input type="checkbox" id="override-cut"> CUT</label>
        <button class="btn btn-sm btn-green" onclick="window.submitOverride()">Apply</button>
        <span style="font-size:11px;color:#555;">Bypasses ESPN — writes directly to Firebase</span>
        ${Object.keys(live.manualOverrides||{}).length ? `
        <div style="width:100%;margin-top:6px;border-top:1px solid #222;padding-top:6px;display:flex;flex-wrap:wrap;gap:6px;align-items:center;">
          <span style="font-size:10px;color:#666;font-weight:700;">ACTIVE OVERRIDES:</span>
          ${Object.entries(live.manualOverrides||{}).map(([key,name]) =>
            `<span style="display:inline-flex;align-items:center;gap:4px;background:#1a1a1a;border:1px solid #333;border-radius:4px;padding:2px 6px;font-size:11px;color:#e65100;">
              ${name}
              <button onclick="window.clearOverride('${key}','${name}')" style="background:none;border:none;color:#666;cursor:pointer;font-size:11px;padding:0 0 0 2px;">✕</button>
            </span>`
          ).join('')}
        </div>` : ''}
      </div>` : ''}
  ...
  // Build trend data from scoreHistory (last 2 snapshots)
  const hasTrend = scoreHistory.length >= 2;
  const prevSnap = hasTrend ? scoreHistory[scoreHistory.length - 2] : null;
  const currSnap = hasTrend ? scoreHistory[scoreHistory.length - 1] : null;
  const prevRanking = hasTrend
    ? [...owners()].sort((a,b) => {
        const sa = prevSnap.scores[a], sb = prevSnap.scores[b];
  ...
  const cards = lbData.map((entry, i) => {
    ...
    // Build round pill for any golfer — always renders, shows tee time or dashes if no data
    const buildRoundPill = (g, isCut) => {
      if (isCut) return `<span class="round-score-box" style="color:#ccc;">—</span>`;
      const rs = g.roundScore !== null && g.roundScore !== undefined ? fmtScore(g.roundScore) : null;
      const rsCls = g.roundScore < 0 ? 'under' : g.roundScore > 0 ? 'over' : '';
      const thruStr = g.thru || null;
      // Not started — show tee time if available
      if (rs === null && !thruStr) {
        if (g.teeTime) {
          // Format tee time: ESPN returns ISO or "HH:MM AM/PM" — show short version
          let tt = g.teeTime;
          try {
            const d = new Date(tt);
            if (!isNaN(d)) tt = d.toLocaleTimeString([], {hour:'numeric', minute:'2-digit', timeZone:'America/New_York'});
          } catch(e) {}
          return `<span class="round-score-box" style="color:#aaa;font-size:9px;">${tt}</span>`;
        }
        return `<span class="round-score-box" style="color:#ccc;">—</span>`;
      }
      return `<span class="round-score-box">${rs !== null ? `<span class="rc-score-val ${rsCls}" style="font-size:10px;">${rs}</span>` : ''}${thruStr ? `<span style="color:#bbb;${rs!==null?'margin-left:2px;':''}">${thruStr}</span>` : ''}</span>`;
    };
    ...
    let altRow = '';
    if (alt) {
      const isBenched = alt.benchedOut;
      const altSv = alt.cut ? 'CUT' : (alt.liveScore !== undefined ? fmtScore(alt.liveScore) : '—');
      const altCls = !isBenched && alt.liveScore < 0 ? 'under' : !isBenched && alt.liveScore > 0 ? 'over' : '';
      const altLabel = isBenched ? 'Subbed' : 'A';
      const altLabelColor = isBenched ? '#aaa' : '#856404';
      altRow = `<div class="rc-player alt ${alt.cut?'cut':''}" style="${isBenched?'color:#bbb;':''}">
        <span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><span class="alt-lbl" style="color:${altLabelColor}">${altLabel}</span>${alt.name}</span>
        ${buildRoundPill(alt, alt.cut)}
    ...
    const makeupBtnHtml = hasMakeup && isComm
      ? `<div class="sub-btn-wrap"><button class="btn" style="background:#c62828;color:#fff;" onclick="window.openMakeupModal('${entry.owner}')">⚠ Make Pick for ${entry.owner}</button></div>`
      : hasMakeup
      ? `<div class="sub-btn-wrap"><button class="btn btn-amber" onclick="window.openMakeupModal('${entry.owner}')">Make Pick</button></div>`
      : '';

    const subBtnHtml = !isFinal && subOpen && !entry.hasSubbed && entry.alternate && !entry.alternate.benchedOut
      ? `<div class="sub-btn-wrap"><button class="btn btn-amber" onclick="window.openSubModal('${entry.owner}')">Sub In Alternate ($${subRound===1?5:15})</button></div>` : '';
    ...
    const pickPill = pickNum > 0 ? `<span style="display:inline-block;font-size:9px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;padding:2px 6px;border-radius:20px;background:#f0f0f0;color:#888;float:right;">pick #${pickNum}</span>` : '';

    // Trend lines — compare current snapshot to previous
    let trendHtml = '';
    if (hasTrend && !entry.out && entry.teamScore !== null) {
      const currScore = currSnap?.scores?.[entry.owner];
      const prevScore = prevSnap?.scores?.[entry.owner];
      const leaderCurr = currSnap?.scores?.[lbData[0]?.owner];
      const leaderPrev = prevSnap?.scores?.[lbData[0]?.owner];
      const prevRank = prevRanking ? prevRanking.indexOf(entry.owner) : -1;
      const rankChange = prevRank >= 0 ? prevRank - i : 0; // positive = moved up

      // Score delta since last snapshot
      let scoreDeltaHtml = '';
      if (currScore !== null && currScore !== undefined && prevScore !== null && prevScore !== undefined) {
        const delta = currScore - prevScore;
        if (delta !== 0) {
          const sign = delta < 0 ? '▼' : '▲';
          const col = delta < 0 ? '#2a9d5c' : '#e05252'; // under par = good = green
          scoreDeltaHtml = `<div class="rc-trend-row" style="color:${col};">${sign} ${Math.abs(delta)} strokes ${delta < 0 ? 'gained' : 'lost'}</div>`;
        } else {
          scoreDeltaHtml = `<div class="rc-trend-row" style="color:#888;">— no change</div>`;
        }
      }
      ...
      let gapHtml = '';
      if (i === 0) {
        // Leader: did gap to 2nd grow or shrink?
        const second = lbData[1];
        if (second && !second.out && second.teamScore !== null) {
          const currGap = (entry.teamScore||0) - (second.teamScore||0);
          const prevGap = prevRanking
            ? (prevSnap?.scores?.[entry.owner]||0) - (prevSnap?.scores?.[second.owner]||0)
            : currGap;
          const ext = currGap - prevGap;
          if (ext < 0) gapHtml = `<div class="rc-trend-row" style="color:#2a9d5c;">▲ extended lead by ${Math.abs(ext)}</div>`;
          else if (ext > 0) gapHtml = `<div class="rc-trend-row" style="color:#e05252;">▼ lead cut by ${ext}</div>`;
          else gapHtml = `<div class="rc-trend-row" style="color:#888;">— lead unchanged</div>`;
        }
      } else {
        const currGap = (entry.teamScore||0) - (lbData[0]?.teamScore||0);
        const prevGap = leaderPrev !== undefined && prevScore !== undefined ? (prevScore||0) - (leaderPrev||0) : currGap;
        const delta = prevGap - currGap; // positive = closed gap = good
        if (delta > 0) gapHtml = `<div class="rc-trend-row" style="color:#2a9d5c;">▲ gained ${delta} on leader</div>`;
        else if (delta < 0) gapHtml = `<div class="rc-trend-row" style="color:#e05252;">▼ lost ${Math.abs(delta)} to leader</div>`;
        else gapHtml = `<div class="rc-trend-row" style="color:#888;">— gap unchanged</div>`;
      }
      ...
      if (prevRank >= 0) {
        if (rankChange > 0) placeHtml = `<div class="rc-trend-row" style="color:#2a9d5c;">▲ up to ${ordinal(i)}</div>`;
        else if (rankChange < 0) placeHtml = `<div class="rc-trend-row" style="color:#e05252;">▼ down to ${ordinal(i)}</div>`;
        else placeHtml = `<div class="rc-trend-row" style="color:#888;">— held ${ordinal(i)}</div>`;
      }

      // If all three are "no change" dashes, collapse to single line
      const allFlat = [scoreDeltaHtml, gapHtml, placeHtml].every(h => h.includes('— '));
      if (allFlat) {
        trendHtml = `<div class="rc-trend"><div class="rc-trend-row" style="color:#bbb;">— no change since last update</div></div>`;
      } else if (scoreDeltaHtml || gapHtml || placeHtml) {
        trendHtml = `<div class="rc-trend">${scoreDeltaHtml}${gapHtml}${placeHtml}</div>`;
      }
    }
    ...
    <div class="main">
      ...
      <div class="round-bar">
        ...
        <div style="font-size:12px;color:#888;display:flex;align-items:center;gap:8px;">
          <span>${owners().length} players · ${subs.length} sub${subs.length!==1?'s':''} used</span>
          <div style="display:flex;gap:3px;">
            <button class="btn btn-sm" style="padding:2px 8px;font-size:10px;${lbSortMode==='score'?'background:#1a1a1a;color:#fff;':'background:#f0f0f0;color:#666;border:none;'}" onclick="window.setLbSort('score')">Score</button>
            <button class="btn btn-sm" style="padding:2px 8px;font-size:10px;${lbSortMode==='pick'?'background:#1a1a1a;color:#fff;':'background:#f0f0f0;color:#666;border:none;'}" onclick="window.setLbSort('pick')">Pick Order</button>
          </div>
        </div>
      </div>
      <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:8px;margin-top:4px;">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#aaa;">Leaderboard</div>
      </div>
...
function buildLeaderboard() {
  ...
  return owners().map(owner => {
    ...
    // Apply live scores — including round score, thru, teeTime, position
    displayGolfers = displayGolfers.map(g => ({
      ...g,
      liveScore: scores[g.name]?.score,
      roundScore: scores[g.name]?.roundScore ?? null,
      thru: scores[g.name]?.thru || null,
      teeTime: scores[g.name]?.teeTime || null,
      position: scores[g.name]?.position || null,
    ...
    if (alternate) {
      alternate = {
        ...
        roundScore: scores[alternate.name]?.roundScore ?? null,
        thru: scores[alternate.name]?.thru || null,
        teeTime: scores[alternate.name]?.teeTime || null,
...
window.refreshScores = async function(explicitId) {
  ...
  try {
    ...
    // Strip any manually overridden players so we don't clobber them
    const overrides = live?.manualOverrides || {};
    Object.keys(overrides).forEach(key => delete matched[key]);
    // Write scores only — no chart snapshot, no timer reset
    // The 15-min server poller owns snapshot writes
    const scoreUpdates = {};
    Object.entries(matched).forEach(([k,v]) => { scoreUpdates[`scores/${k}`] = v; });
    scoreUpdates['lastUpdated'] = new Date().toISOString();
    await update(liveRef(), scoreUpdates);
...
window.toggleScoreOverride = function() {
  showOverridePanel = !showOverridePanel;
  renderScreen();
};

window.submitOverride = async function() {
  const playerName = document.getElementById('override-player')?.value?.trim();
  const score = document.getElementById('override-score')?.value;
  const cut = document.getElementById('override-cut')?.checked;
  if (!playerName || score === '') { alert('Enter a player name and score.'); return; }
  const data = await proxyApi('/scores/override', 'POST', { playerName, score: parseInt(score,10), cut });
  if (data.ok) showToast(`✓ ${playerName} set to ${data.display}${cut?' (CUT)':''}`);
  else alert('Override failed: ' + (data.error||'unknown error'));
};

window.clearOverride = async function(safeKey, playerName) {
  await proxyApi('/scores/override-clear', 'POST', { safeKey });
  showToast(`↩ Override cleared for ${playerName} — ESPN will update on next poll`);
};
...
window.setLbSort = function(mode) {
  lbSortMode = mode;
  renderScreen();
};

window.undoSub = async function() {
  const existingSubs = Array.isArray(live?.subs) ? live.subs : Object.values(live?.subs || {});
  if (!existingSubs.length) return;
  const lastSub = existingSubs[existingSubs.length - 1];
  if (!confirm(`Undo sub: ${lastSub.owner} — ${lastSub.from} → ${lastSub.to} (+$${lastSub.cost})?`)) return;
  const newSubs = existingSubs.slice(0, -1);
  await update(liveRef(), { subs: newSubs, pot: (live?.pot || 0) - lastSub.cost });
  showToast(`↩ Sub undone for ${lastSub.owner}`);
};
...
window.doFinalize = async function() {
  ...
  owners().forEach(owner => {
    ...
    const picks = draft?.picks?.[owner] || { golfers: [], alternate: null };
    const ownerSub = subs.find(s => s.owner === owner);
    const golfers = picks.golfers
      .filter(g => ownerSub?.from !== g.name) // remove the subbed-out player from main roster
      .map(g => {
        const ls = live?.scores?.[g.name];
        return { name: g.name, ...(ls?.cut?{score:'CUT',cut:true}:{score:ls?.score??0}) };
      });
    if (ownerSub) {
    ...
    // If a sub happened: store the swapped-out golfer in the alternate slot (benchedOut:true)
    // so golf.html can display them in the alt row. The alternate is already pushed into golfers[] with sub:true.
    // If no sub: store the alternate normally.
    const alternate = ownerSub
      ? { name: ownerSub.from, benchedOut: true, ...(live?.scores?.[ownerSub.from]?.cut ? {score:'CUT',cut:true} : {score: live?.scores?.[ownerSub.from]?.score ?? 0}) }
      : { name: altName, ...(altLive?.cut?{score:'CUT',cut:true}:{score:altLive?.score??0}) };
    ownersData[owner] = { golfers, alternate, score: entry?.out?'OUT':(entry?.teamScore??0) };
  });
  // Build pick order map { ownerName: pickNumber } — matches GOLF_DRAFT shape for history page
  const pickOrder = {};
  (draft?.pickSequence || []).forEach((p, i) => { pickOrder[p.owner] = i + 1; });

  const entry = { slug:SLUG, name:draft?.name||SLUG, year:new Date().getFullYear(), pot:live?.pot||(owners().length*25), winner, owners:ownersData, pickOrder };
    ...
    <div style="margin-bottom:10px;">
      ...
      <div style="display:flex;gap:6px;">
        <select id="sel-${p.name.replace(/\s/g,'_')}" style="flex:1;border:1px solid #ddd;border-radius:5px;padding:4px 8px;font-size:12px;">
          ...
          ${(data.availableOdds||[]).map(o=>`<option value="${o.name}">${o.name} (Win:${o.dk||'—'} T10:${o.dk_top10||'—'} Cut:${o.dk_cut||'—'})</option>`).join('')}
        </select>
  ...
  try {
    ...
    if (data.error) {
      document.getElementById('odds-content').innerHTML =
        ...
        <button class="btn btn-outline btn-sm" style="margin-top:12px;display:block;" onclick="document.getElementById('odds-modal').style.display='none'">Close</button>`;
      return;
    }
    showOddsResult(data);
  } catch(e) {
