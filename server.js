...
// Combined endpoint — pts + eliminated in one request
app.get('/all', async (req, res) => {
  try {
    const [ptsResult, elimResult] = await Promise.all([
    ...
    while ((m = seriesRegex.exec(elimHtml)) !== null) {
      ...
      const abb = TEAM_ABB[loser];
      if (abb) eliminated.add(abb);
      else console.log('No abbreviation found for:', loser);
    }

    console.log(`Players: ${Object.keys(players).length}, Eliminated: ${[...eliminated].join(',')}`);
    res.json({
