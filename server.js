...
app.get('/all', async (req, res) => {
  try {
    ...
    // Parse eliminated
    const eliminated = new Set();
    // Parse series standings: track wins for each team
    // seriesStandings[teamAbb] = net lead (positive = leading, negative = trailing)
    const seriesStandings = {};
    const elimHtml = elimResult.body;

    // Match completed series: "Team defeated Team, 4-N"
    const seriesRegex = /([A-Za-z\s\.]+?)\s+defeated\s+([A-Za-z\s\.]+?),\s*4-(\d)/gi;
    let m;
    while ((m = seriesRegex.exec(elimHtml)) !== null) {
      const winner = m[1].trim().toLowerCase().replace(/\.$/, '');
      const loser  = m[2].trim().toLowerCase().replace(/\.$/, '');
      const loserWins = parseInt(m[3], 10);
      const winnAbb = TEAM_ABB[winner];
      const losrAbb = TEAM_ABB[loser];
      if (losrAbb) eliminated.add(losrAbb);
      if (winnAbb) seriesStandings[winnAbb] = 4;  // won
      if (losrAbb) seriesStandings[losrAbb] = -4; // lost
    }

    // Match in-progress series scores like "3-1" or "2-2" in series result text
    // BBRef shows series as "Team A leads, N-N" or just score in series pages
    // Look for patterns: team name followed by series record
    const inProgressRegex = /([A-Za-z\s]+?)\s+lead[s]?,?\s+(\d)-(\d)/gi;
    while ((m = inProgressRegex.exec(elimHtml)) !== null) {
      const leader = m[1].trim().toLowerCase();
      const leaderWins = parseInt(m[2], 10);
      const trailerWins = parseInt(m[3], 10);
      const leaderAbb = TEAM_ABB[leader];
      if (leaderAbb && !seriesStandings[leaderAbb]) {
        seriesStandings[leaderAbb] = leaderWins - trailerWins; // positive = leading
      }
    }
    console.log('Series standings:', JSON.stringify(seriesStandings));
